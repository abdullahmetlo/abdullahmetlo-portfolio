...
import { Award, CheckCircle2, ChevronLeft, ChevronRight, ExternalLink, ShieldCheck, Sparkles } from "lucide-react";
...
export default function Certifications() {
  ...
  return (
    <section id="certifications" className="py-24 relative overflow-hidden">
      {/* Glow */}
      <div className="absolute top-1/2 left-1/4 w-80 h-80 bg-emerald-600/10 blur-[130px] rounded-full -z-10" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
          ...
          <div className="flex items-center gap-2 self-start md:self-auto">
            <button
              onClick={() => scroll("left")}
              className="p-3 rounded-xl bg-zinc-900 border border-zinc-800 text-zinc-300 hover:text-white hover:border-zinc-700 hover:bg-zinc-800 transition-colors"
        ...
        >
          {certifications.map((cert: Certification) => (
            <div
              key={cert.id}
              className="min-w-[300px] sm:min-w-[340px] max-w-[340px] bg-zinc-950/85 border border-zinc-800/90 hover:border-emerald-500/50 rounded-2xl p-6 shadow-xl shadow-black/80 flex flex-col justify-between snap-start group transition-all duration-300 relative overflow-hidden"
            >
              {/* Top Accent line */}
              ...
              <div>
                ...
                {/* Certificate Title */}
                <h3 className="text-base font-bold text-white group-hover:text-emerald-400 transition-colors mb-1.5 leading-snug">
                  {cert.title}
                </h3>
